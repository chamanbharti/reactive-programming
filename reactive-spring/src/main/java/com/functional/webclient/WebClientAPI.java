...
public class WebClientAPI {
	...
	public WebClientAPI() {
		//this.webClient = WebClient.create("http://localhost:8080/products/1");
	...
	private Mono<ResponseEntity<Product>> postNewProduct(){
		return webClient
				.post()
				.body(
						Mono.just(new Product(null,"Black Tea",1.99)),Product.class
					)
				.exchange()
	...
	private Mono<Product> updateProduct(String id, String name, double price){
		return webClient
				...
				.uri("/{id}",id)
				.body(
						Mono.just(new Product(null,name, price)),Product.class
					)
				.retrieve()
